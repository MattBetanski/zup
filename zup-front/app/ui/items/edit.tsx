...
import { createItem, CreateItemState, editItem, EditItemState } from "@/app/lib/items/action";
...
export default function EditItemForm({item}: {item: Item}) {
    ...
    const [type, setType] = useState(ItemType.Epic);
    ...
    useEffect(() => {
        const fetchStuff = async () => {
            try{
                let items = await getItemsForType(type, item.projectId);
    ...
    function handleTypeChange(event: React.ChangeEvent<HTMLSelectElement>) {
        ...
        const newType: ItemType = ItemType[newValue]
        setType(newValue);
    ...
    const initialState: EditItemState = {message: null, errors: {}};
    const [state, dispatch] = useFormState(editItem, initialState);
    return (
        <form action={dispatch}>
            <input hidden name="projectId" type="number" value={item.projectId} readOnly/>
            <input hidden name="itemId" type="number" value={item.itemId} readOnly/>
            <input hidden name="ownerId" type="number" value={item.ownerId} readOnly/>
